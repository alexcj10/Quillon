...
- **Blue Tags (Folders)**: Tags starting with `file` (e.g., `fileProject`) are treated as **main folders**.
- Example:
  - `fileProject` → creates a folder named **Project**
- This system is **case-sensitive**:
  - `fileProject` ≠ `fileproject`
  - `Project` and `project` are treated as **two different folders**
