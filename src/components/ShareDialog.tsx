...
export function ShareDialog({ note, onClose }: ShareDialogProps) {
  const handleShare = async (platform: string) => {
    ...
    try {
      switch (platform) {
        ...
        case 'linkedin':
          window.open(
            `https://www.linkedin.com/sharing/share-offsite/?text=${encodeURIComponent(shareText)}`,
            '_blank'
          );
          break;
      }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full mx-4 md:mx-8 max-w-md">
